...
from data_fetchers import fetch_yf_data_for_ticker, fetch_yf_data, fetch_coingecko_data, fetch_binance_crypto_data, fetch_fx_daily_data, CurrencyDataSource, StockDataSource, CryptoDataSource # Added fetch_fx_daily_data, CurrencyDataSource etc
from utils import safe_convert, get_currency_list # Added get_currency_list
from sqlalchemy.exc import SQLAlchemyError
import logging
from config import REGULAR_TTL, HISTORICAL_START_DATE, REQUEST_DELAY_SECONDS, MAX_TICKERS_PER_REQUEST, TOP_N_TICKERS # Import necessary configs
...
def update_stock_asset_and_quote(quotes_df, historical_data, upsert=False):
    ...
    try:
        ...
        usd_asset = session.query(CurrencyAsset).filter_by(symbol="USD").first()
        if not usd_asset:
             usd_asset = CurrencyAsset(asset_type="CURRENCY", symbol="USD", name="US Dollar", source_asset_key="USD")
        ...
        if not tickers_to_process:
             logger.warning("No valid stock tickers found in quotes_df to process.")
             return

        logger.info(f"Processing {len(tickers_to_process)} stock tickers...")

        # Preload existing stock assets and asset quotes
        existing_assets = { asset.symbol: asset for asset in session.query(StockAsset).filter(StockAsset.symbol.in_(tickers_to_process)).all() }
        logger.info(f"Found {len(existing_assets)} existing stock assets in DB.")
        ...
        for idx, row in quotes_df.iterrows():
            ...
            if exchange == "SET": currency = "THB"
            # Add more mappings if needed
            ticker_currency_map[ticker] = currency
            required_currencies.add(currency)

        # Preload required currency assets
        existing_currency_assets = { asset.symbol: asset for asset in session.query(CurrencyAsset).filter(CurrencyAsset.symbol.in_(list(required_currencies))).all() }
        logger.info(f"Found {len(existing_currency_assets)} existing currency assets for quoting.")

        # Preload existing asset quotes
        composite_tickers = [t + ticker_currency_map.get(t, 'USD') for t in tickers_to_process]
        ...
        logger.info(f"Found {len(existing_asset_quotes)} existing relevant asset quotes.")
        ...
        for idx, row in quotes_df.iterrows():
            ...
            first_trade_raw = row.get("firstTradeDateEpochUtc") # YF uses this now
            first_trade = None
            if first_trade_raw:
                 try:
                     # Convert from seconds timestamp
                     first_trade = datetime.datetime.utcfromtimestamp(first_trade_raw).date()
                 except Exception:
                     logger.warning(f"Could not parse first_trade_date for {ticker}: {first_trade_raw}")
            ...
            if not asset:
                ...
                session.add(asset)
                existing_assets[ticker] = asset # Add to map for quote creation
            ...
            if not currency_asset:
                 ...
                 currency_asset = CurrencyAsset(asset_type="CURRENCY", symbol=quote_currency_code, name=f"{quote_currency_code} (Auto-created)", source_asset_key=quote_currency_code)
                 session.add(currency_asset)
                 existing_currency_assets[quote_currency_code] = currency_asset
                 logger.warning(f"Auto-created missing CurrencyAsset: {quote_currency_code}")
            ...
            if ticker in historical_data and historical_data[ticker] is not None and not historical_data[ticker].empty:
                ...
                min_date = df.index.min().to_pydatetime().replace(hour=0, minute=0, second=0, microsecond=0)
                max_date = df.index.max().to_pydatetime().replace(hour=0, minute=0, second=0, microsecond=0)
                ...
                logger.debug(f"Found {len(existing_ohlcv_for_quote)} existing OHLCV records for {composite_ticker} in date range.")
                ...
                for date_idx, row_data in df.iterrows():
                    ...
                    if price_date.tzinfo:
                        price_date = price_date.tz_convert('UTC').replace(tzinfo=None)
                    ...
                    ohlcv_data = {
                         ...
                         "volume": safe_decimal(row_data.get("Volume"), precision='0.01') # Volume precision can be lower
                    }
                    ...
                    if existing_record:
                        ...
                        # New record
                        new_map = {"asset_quote_id": asset_quote.id, "price_date": price_date}
                        new_map.update(ohlcv_data)
                        new_records_maps.append(new_map)

                if new_records_maps:
                    try:
                        session.bulk_insert_mappings(AssetOHLCV, new_records_maps)
                        logger.debug(f"Bulk inserted {len(new_records_maps)} OHLCV for {composite_ticker}.")
                        total_new_ohlcv += len(new_records_maps)
                    except SQLAlchemyError as bulk_e:
                         logger.error(f"Error bulk inserting OHLCV for {composite_ticker}: {bulk_e}")
                ...
                if update_mappings:
                    try:
                         ...
                         logger.debug(f"Bulk updated {len(update_mappings)} OHLCV for {composite_ticker}.")
                         total_updated_ohlcv += len(update_mappings)
                    except SQLAlchemyError as bulk_e:
                         logger.error(f"Error bulk updating OHLCV for {composite_ticker}: {bulk_e}")
                         session.rollback() # Rollback partial bulk update
                         raise
            else:
                 logger.warning(f"No historical data found or provided for stock ticker: {ticker}")

        session.commit()
        logger.info(f"Stock asset/quote/OHLCV update finished. New OHLCV: {total_new_ohlcv}, Updated OHLCV: {total_updated_ohlcv}.")

    except SQLAlchemyError as e:
        session.rollback()
        logger.error(f"Database error during stock update: {e}\n{traceback.format_exc()}")
        raise
    except Exception as e:
        session.rollback()
        logger.error(f"Unexpected error during stock update: {e}\n{traceback.format_exc()}")
        raise
    finally:
        session.close()
...
def update_crypto_asset_and_quote(crypto_data, historical_data, upsert=False):
    ...
    try:
        ...
        if not data_source:
            data_source = DataSource(name="Binance", description="Binance API data source (for OHLCV)")
            session.add(data_source)
            session.flush()
            logger.info("Created DataSource: Binance")

        # Preload USDT currency asset
        usdt_asset = session.query(Asset).filter(Asset.asset_type=="CURRENCY", Asset.symbol=="USDT").first() # Use Asset base class
        if not usdt_asset:
             # USDT itself is technically often a crypto asset, but we treat it as the quote CURRENCY here
             usdt_asset = CurrencyAsset(asset_type="CURRENCY", symbol="USDT", name="Tether USD", source_asset_key="USDT")
        ...
        if not coin_ids_to_process:
             logger.warning("No valid crypto IDs found in crypto_data to process.")
             return

        logger.info(f"Processing {len(coin_ids_to_process)} crypto assets...")

        coin_id_map = {c['id']: c for c in crypto_data if c.get('id')}
        coin_symbols = [c.get("symbol", "").upper() for c in crypto_data if c.get("symbol")]

        # Preload existing crypto assets and asset quotes
        existing_assets = { asset.symbol: asset for asset in session.query(CryptoAsset).filter(CryptoAsset.symbol.in_(coin_symbols)).all() }
        logger.info(f"Found {len(existing_assets)} existing crypto assets in DB.")

        composite_tickers = [symbol + "USDT" for symbol in coin_symbols]
        existing_asset_quotes = { aq.ticker: aq for aq in session.query(AssetQuote).filter(AssetQuote.ticker.in_(composite_tickers), AssetQuote.data_source_id == data_source.id).all() }
        logger.info(f"Found {len(existing_asset_quotes)} existing relevant crypto asset quotes.")
        ...
        for coin_id in coin_ids_to_process:
             ...
             if not coin: continue # Should not happen
             ...
             ath_date = pd.to_datetime(coin.get("ath_date"), errors='coerce').to_pydatetime() if coin.get("ath_date") else None
             atl_date = pd.to_datetime(coin.get("atl_date"), errors='coerce').to_pydatetime() if coin.get("atl_date") else None
             ...
             if not asset:
                 ...
                 logger.debug(f"Adding new CryptoAsset: {coin_symbol}")
                  ...
                  asset.total_supply = safe_convert(coin.get("total_supply"), int)
             ...
             if coin_id in historical_data and historical_data[coin_id] is not None and not historical_data[coin_id].empty:
                 ...
                 logger.warning(f"No historical data found or provided for crypto ID: {coin_id} (Symbol: {coin_symbol})")


        session.commit()
        logger.info(f"Crypto asset/quote/OHLCV update finished. New OHLCV: {total_new_ohlcv}, Updated OHLCV: {total_updated_ohlcv}.")

    except SQLAlchemyError as e:
        session.rollback()
        logger.error(f"Database error during crypto update: {e}\n{traceback.format_exc()}")
        raise
    except Exception as e:
        session.rollback()
        logger.error(f"Unexpected error during crypto update: {e}\n{traceback.format_exc()}")
        raise
    finally:
        session.close()
...
def update_currency_asset_and_quote(currency_list, historical_data, upsert=False):
    ...
    try:
        ...
        existing_assets = { asset.symbol: asset for asset in session.query(CurrencyAsset).filter(CurrencyAsset.symbol.in_(currency_codes)).all() }
        logger.info(f"Found {len(existing_assets)} existing currency assets in DB.")

        composite_tickers = [code + "USD" for code in currency_codes if code != "USD"] # Don't create USDUSD
        existing_asset_quotes = { aq.ticker: aq for aq in session.query(AssetQuote).filter(AssetQuote.ticker.in_(composite_tickers), AssetQuote.data_source_id == data_source.id).all() }
        logger.info(f"Found {len(existing_asset_quotes)} existing relevant currency asset quotes.")
        ...
        for currency_code, currency_name in currency_list:
            if not currency_code or not currency_name: continue
            ...
            if not asset:
                ...
                logger.debug(f"Updating existing CurrencyAsset: {currency_code}")

            # --- AssetQuote Update/Insert (Skip for USD itself) ---
            if currency_code == "USD":
                 logger.debug("Skipping AssetQuote creation for USD against USD.")
                 continue
            ...
            if currency_code in historical_data and historical_data[currency_code] is not None and not historical_data[currency_code].empty:
                ...
                for date_idx, row_data in df.iterrows():
                    ...
                    price_date = date_idx.to_pydatetime().replace(hour=0, minute=0, second=0, microsecond=0)
                ...
                logger.warning(f"No historical data found or provided for currency: {currency_code}")


        session.commit()
        logger.info(f"Currency asset/quote/OHLCV update finished. New OHLCV: {total_new_ohlcv}, Updated OHLCV: {total_updated_ohlcv}.")

    except SQLAlchemyError as e:
        session.rollback()
        logger.error(f"Database error during currency update: {e}\n{traceback.format_exc()}")
        raise
    except Exception as e:
        session.rollback()
        logger.error(f"Unexpected error during currency update: {e}\n{traceback.format_exc()}")
        raise
    finally:
        session.close()
...
def _sync_common_update_state(sync_type="full"):
    ...
    try:
        ...
        session.rollback()
        logger.error(f"Error updating delta sync state after {sync_type} sync: {e}")
...
def full_sync_stocks():
    ...
    try:
        ...
        logger.info(f"Fetched data for Thai stocks: {len(quotes_df_th)} quotes.")
        ...
        update_stock_asset_and_quote(quotes_df, historical_data, upsert=True) # Use upsert=True for simplicity
        _sync_common_update_state(sync_type="full")
        logger.info("Global Full Sync for Stocks Completed.")
    except Exception as e:
         logger.error(f"Global Full Sync for Stocks Failed: {e}\n{traceback.format_exc()}")
...
def delta_sync_stocks():
    ...
    try:
        ...
        end_date_str = (today + datetime.timedelta(days=1)).strftime("%Y-%m-%d") # Include today

        # Fetch tickers that are already in the DB to delta sync them
        session = Session()
        try:
             stock_tickers_th = [a.symbol for a in session.query(StockAsset.symbol).filter(StockAsset.region=='th').all()]
             stock_tickers_us = [a.symbol for a in session.query(StockAsset.symbol).filter(StockAsset.exchange.in_(['NMS', 'NYQ'])).all()]
        finally:
             session.close()

        logger.info(f"Found {len(stock_tickers_th)} Thai and {len(stock_tickers_us)} US tickers in DB for delta sync.")
        ...
        if stock_tickers_th:
            ...
            if not quotes_df_th.empty: all_quotes_list.append(quotes_df_th)
            historical_data_combined.update(hist_th)
            logger.info(f"Delta sync - Fetched TH: {len(quotes_df_th)} quotes, {len(hist_th)} historical.")
        ...
        if stock_tickers_us:
            ...
            if not quotes_df_us.empty: all_quotes_list.append(quotes_df_us)
            historical_data_combined.update(hist_us)
            logger.info(f"Delta sync - Fetched US: {len(quotes_df_us)} quotes, {len(hist_us)} historical.")
        ...
        update_stock_asset_and_quote(quotes_df_combined, historical_data_combined, upsert=True)
        _sync_common_update_state(sync_type="delta")
        logger.info("Global Delta Sync for Stocks Completed.")
    except Exception as e:
         logger.error(f"Global Delta Sync for Stocks Failed: {e}\n{traceback.format_exc()}")
...
def full_sync_crypto():
    ...
    try:
        ...
        if not crypto_data:
             logger.warning("No crypto metadata fetched from CoinGecko. Aborting full sync.")
             return
        ...
        for coin in crypto_data:
            ...
            if not coin_id or not symbol: continue

            binance_symbol = symbol + "USDT"
            logger.debug(f"Fetching full history for crypto: {binance_symbol} (ID: {coin_id})")

            df = fetch_binance_crypto_data(binance_symbol, HISTORICAL_START_DATE, None)
            if df is not None: # Store even if empty, but not on error (None)
                 ...
                 logger.error(f"Failed to fetch historical data for {binance_symbol}")
        ...
        update_crypto_asset_and_quote(crypto_data, historical_data, upsert=True) # Use upsert=True
        _sync_common_update_state(sync_type="full")
        logger.info("Global Full Sync for Cryptocurrencies Completed.")
    except Exception as e:
         logger.error(f"Global Full Sync for Cryptos Failed: {e}\n{traceback.format_exc()}")
...
def delta_sync_crypto():
    ...
    try:
        ...
        if not crypto_data:
             logger.warning("No crypto metadata fetched from CoinGecko. Aborting delta sync.")
             return
        ...
        for coin in crypto_data:
            ...
            binance_symbol = symbol + "USDT"
            logger.debug(f"Fetching delta history for crypto: {binance_symbol} (ID: {coin_id})")

            df = fetch_binance_crypto_data(binance_symbol, start_date_str, end_date_str)
            if df is not None:
                historical_data[coin_id] = df
                logger.debug(f"Fetched {len(df)} delta records for {binance_symbol}")
            else:
                 logger.error(f"Failed to fetch delta historical data for {binance_symbol}")
        ...
        logger.info("Global Delta Sync for Cryptocurrencies Completed.")
         ...
         logger.error(f"Global Delta Sync for Cryptos Failed: {e}\n{traceback.format_exc()}")
...
def full_sync_currency(ticker=None):
    ...
    try:
        ...
        for currency_code, _ in currency_list:
            logger.debug(f"Fetching full FX history for currency: {currency_code}")

            if currency_code.upper() == "USD":
                # Create dummy data for USD -> USD rate = 1
                date_range = pd.date_range(start=HISTORICAL_START_DATE, end=datetime.datetime.now().date(), freq='D')
                df = pd.DataFrame(1.0, index=date_range, columns=["Open", "High", "Low", "Close"])
                historical_data[currency_code] = df
                logger.debug(f"Generated dummy history for USD.")
            else:
                 df = fetch_fx_daily_data(currency_code, "USD", outputsize="full") # Fetch full history
                 if df is None: # Error occurred during fetch
                     ...
                     continue
                      ...
                      logger.warning(f"Empty historical data returned for {currency_code}/USD")

                 historical_data[currency_code] = df
                 logger.debug(f"Fetched {len(df)} records for {currency_code}/USD")

            # Respect AlphaVantage rate limits (can be strict, e.g., 5/min)
            time.sleep(15) # Sleep longer for AV free tier


        update_currency_asset_and_quote(currency_list, historical_data, upsert=True) # Use upsert=True
        _sync_common_update_state(sync_type="full")
        if ticker:
         ...
         logger.error(f"Currency Full Sync Failed: {e}\n{traceback.format_exc()}")
...
def delta_sync_currency(ticker=None):
    ...
    try:
        ...
        historical_data = {}
        for currency_code, _ in currency_list:
            logger.debug(f"Fetching delta FX history for currency: {currency_code}")

            if currency_code.upper() == "USD":
                 # Generate dummy data for the last few days
                 date_range = pd.date_range(end=today, periods=3, freq='D') # Just a few days
                 df = pd.DataFrame(1.0, index=date_range, columns=["Open", "High", "Low", "Close"])
                 historical_data[currency_code] = df
                 logger.debug(f"Generated dummy delta history for USD.")
            else:
                df = fetch_fx_daily_data(currency_code, "USD", outputsize=outputsize)
                if df is None:
                    logger.error(f"Failed to fetch delta FX daily data for {currency_code}/USD")
                    continue
                elif df.empty:
                    logger.warning(f"Empty delta historical data returned for {currency_code}/USD")

                historical_data[currency_code] = df
                logger.debug(f"Fetched {len(df)} delta records for {currency_code}/USD")
        ...
        if ticker:
         ...
         logger.error(f"Currency Delta Sync Failed: {e}\n{traceback.format_exc()}")
...
def refresh_stock_top_n_tickers(query_counter, top_n, delay_t):
    ...
    try:
        ...
        top_stock_keys = [t[0] for t, count in query_counter.most_common(top_n * 2) if t[1] == "STOCK"][:top_n] # Get more initially, filter, then limit

        if not top_stock_keys:
             logger.debug("No stock tickers found in query counter for cache refresh.")
             return
        ...
        source_tickers = list(set(aq.source_ticker for aq in asset_quotes if aq.source_ticker)) # Unique source tickers

        if not source_tickers:
             logger.debug("No source tickers found for top stock keys.")
             return

        logger.debug(f"Found {len(source_tickers)} unique source tickers to refresh: {source_tickers}")
        ...
        for st in source_tickers:
            ...
            if price_result == "NOT_FOUND":
                expires = now + datetime.timedelta(minutes=NOT_FOUND_TTL) # Use specific TTL for not found
                ...
                refreshed_count += 1
                 ...
                 logger.warning(f"Received unexpected price result '{price_result}' for {st} during cache refresh.")

        logger.info(f"Stock cache refresh: Updated {refreshed_count} tickers, marked {not_found_count} as NOT_FOUND.")
        # time.sleep(delay_t) # Delay might not be needed if refresh_latest_prices handles it internally

    except Exception as e:
         logger.error(f"Error refreshing top N stock tickers cache: {e}\n{traceback.format_exc()}")
    finally:
        session.close()
...
def refresh_crypto_prices():
    ...
    try:
        prices = CryptoDataSource.get_all_latest_prices() # This method already updates latest_cache internally
        logger.info(f"Crypto cache refresh: Updated {len(prices)} tickers from Binance.")
    except Exception as e:
        logger.error(f"Error refreshing crypto prices cache: {e}\n{traceback.format_exc()}")
...
def refresh_currency_prices():
    ...
    try:
        currency_list = get_currency_list()
        codes = [code for code, name in currency_list if code] # Get only codes
        if not codes:
        ...
        updated_count = sum(1 for p in prices.values() if p != "NOT_FOUND" and p is not None)
        not_found_count = sum(1 for p in prices.values() if p == "NOT_FOUND")
        logger.info(f"Currency cache refresh: Updated {updated_count} currencies, marked {not_found_count} as NOT_FOUND.")
    except Exception as e:
        logger.error(f"Error refreshing currency prices cache: {e}\n{traceback.format_exc()}")
...
def refresh_all_latest_prices():
    ...
    logger.info(f"Cache refresh cycle completed in {duration:.2f} seconds. Last refresh set to: {last_cache_refresh}")
...
def fetch_yf_data(max_tickers_per_request, delay_t, query, start_date=HISTORICAL_START_DATE, end_date=None, sample_size=None, ticker_list=None):
    ...
    if ticker_list:
         symbols = ticker_list
         logger.info(f"Fetching data for specific ticker list ({len(symbols)} tickers).")
         ...
         if symbols:
              ...
              for symbol in symbols:
                   try:
                        ...
                        logger.warning(f"Could not fetch info for ticker {symbol}: {info_err}")
         ...
         if sample_size is not None:
             result = yf.Tickers.screen(query, offset=0, size=sample_size) # Use class method
             ...
             while True:
                 try:
                     ...
                     all_quotes_list.extend(quotes)
                     offset += size
                     logger.debug(f"Fetched {len(quotes)} quotes, total {offset}...")
                     time.sleep(0.5) # Small delay between screen pages
                 except Exception as screen_err:
                      logger.error(f"Error during yfinance screen fetch at offset {offset}: {screen_err}")
    ...
    logger.info(f"Downloading historical data for {len(symbols_to_download)} symbols...")
    for batch_symbols in chunk_list(symbols_to_download, max_tickers_per_request):
        ...
        try:
            ...
            if data.empty:
                 logger.warning(f"No historical data returned for batch: {tickers_str}")
                 continue

            # Handle MultiIndex vs single ticker response
            if isinstance(data.columns, pd.MultiIndex):
                # Iterate through tickers in the batch, check if data exists
                available_tickers_in_response = data.columns.get_level_values(0).unique().tolist()
                for ticker in batch_symbols:
                    if ticker in available_tickers_in_response:
                        ticker_data = data[ticker].dropna(how='all') # Drop rows with all NaNs
                 ...
                 logger.warning(f"Unexpected data structure returned by yf.download for batch: {tickers_str}")

            logger.debug(f"Downloaded batch ending with {batch_symbols[-1]}. Total historical series: {len(historical_data)}")
    ...
    logger.info(f"Finished historical data download. Fetched for {len(historical_data)} symbols.")
